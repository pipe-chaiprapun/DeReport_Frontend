...
export class ContractComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.resAPI.getContract(158321).subscribe(result => {
      console.log(result);
  ...
  viewPayment(id, con_date, tot_amt, period, pay_amt, discount, balance) {
    this.router.navigate(['/contractpayment', this.cust_name, id, con_date, tot_amt, period, pay_amt, discount, balance]);
  }
  numberWithCommas(x) {
